...
void TCPSender::push( const TransmitFunction& transmit )
{
   while ( ( window_size_ == 0 ? 1 : window_size_ ) > total_outstanding_ ) {
    ...
    uint64_t len = std::min(static_cast<uint64_t>(TCPConfig::MAX_PAYLOAD_SIZE), (window_size_ == 0 ? 1 : window_size_) - total_outstanding_ );
    len = std::min( len, input_.reader().bytes_buffered() );
    for( uint64_t byte = 0; byte < len ; ++byte ) {
    payload += std::string( input_.reader().peek() );
    input_.reader().pop( 1UL ); //把内容都读出来..
  }
    //不知道为什么一定要fin预留一个窗口呢..
    if ( not is_fin_ and remaining > msg.sequence_length() and reader().is_finished() ) {
      msg.FIN = true;
      is_fin_ = true;
    }
    ...
    wait_ack_que_.push( { msg, abs_seqno_ } );
...
{
  // Your code here.
  return TCPSenderMessage{ Wrap32( 0 ).wrap( abs_seqno_ , isn_ ), false, "", false, input_.has_error() };
}
...
void TCPSender::tick( uint64_t ms_since_last_tick, const TransmitFunction& transmit )
{
  ...
  if( Timer_.is_active() ) {
    ...
    if( Timer_.is_expired() && ( !wait_ack_que_.empty() ) ) {
      ...
      //只有窗口不为0的情况下，才能拥塞控制?,如果是窗口！= 0过期了，
      //那就是网络问题，才能放慢速度。
      if( window_size_ ){
        retransmissions += 1UL;
        //wait_ack_que_.pop();
        //发出去不能pop,能不能pop取决取ack
        Timer_.exponential_backoff(); //已经重传成功了，那么拥塞控制。
      }
